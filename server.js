...
viewAllEmployees = async () => {
  ...
  try {
    const [results] = await db
      .promise()
      .query(
        `SELECT employee.id AS id,employee.first_name, employee.last_name, role.title, department.name AS department, role.salary FROM employee LEFT JOIN role ON employee.role_id = role.id LEFT JOIN department ON role.department_id = department.id `
      );
    console.table(results);
    employeeInfo();
  } catch (err) {
...
viewAllRoles = () => {
  ...
  db.query(
    `SELECT role.id AS id, role.title, role.salary FROM role`,
...
addEmployee = async () => {
  const [role] = await db
    .promise()
    .query(`SELECT role.id AS value, role.title AS name FROM role`);
...
updateEmployeeRole = async () => {
  const [employee] = await db
    .promise()
    .query(
      `SELECT employee.id AS value, employee.last_name AS name FROM employee`
    );
